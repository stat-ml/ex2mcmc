...
from source.links.sn_linear import SNLinear, Linear
...
class DRSDiscriminator(chainer.Chain):
    def __init__(self, ch=128, n_classes=0, activation=F.relu):
        super().__init__()
        self.activation = activation
        with self.init_scope():
            ...
            self.block3 = Block(ch, ch, activation=activation, downsample=False)
            self.block4 = Block(ch, ch, activation=activation, downsample=False)
            self.fc1 = SNLinear(ch, ch, initialW=chainer.initializers.GlorotUniform(), nobias=False)
            self.fc2 = SNLinear(ch, ch // 2, initialW=chainer.initializers.GlorotUniform(), nobias=False)
            self.l5 = SNLinear(ch, 1, initialW=chainer.initializers.GlorotUniform(), nobias=True)
            self.fc3 = SNLinear(ch // 2, 1, initialW=chainer.initializers.GlorotUniform(), nobias=False)
            if n_classes > 0:
                self.l_y = SNEmbedID(n_classes, ch, initialW=chainer.initializers.GlorotUniform())
