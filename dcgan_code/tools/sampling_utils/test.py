...
weights_ = np.array([1./n_components] * n_components)
means_init = np.array(list(itertools.product(np.arange(-2,3), repeat=2)), dtype = np.float32)
covs_init = np.array([np.eye(2)*(std ** 2)] * n_components)
#precisions_init = np.array([np.ones((2, 2))*(std ** 2)] * n_components)
precisions_init = np.array([np.linalg.inv(np.eye(2)*(std ** 2))] * n_components)
