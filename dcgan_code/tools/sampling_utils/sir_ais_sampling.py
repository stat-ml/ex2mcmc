...
def run_experiments_gaussians(dim_arr,  
    ...
    for dim in dim_arr:
        ...
        if method == 'sir_correlated':
            ...
            (history,
             acceptence) = (ebm_sampling
                            .citerais_mala_dynamics(start,
                                                    target.log_prob,
                                                    method_params['n_steps'],
            ...
            (history,
             acceptence, _) = (ebm_sampling
                               .citerais_ula_dynamics(start,
                                                      target.log_prob,
                                                      proposal,
                                                      method_params['n_steps'],
                                                      method_params['grad_step'],
...
def run_experiments_2_gaussians(dim_arr,  
    ...
    for dim in dim_arr:
        ...
        evolution = Evolution(None, locs=torch.stack(locs, 0).cpu(),
                              sigma=scale_target)
