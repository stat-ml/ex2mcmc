    ...
    def train_epoch(self):
        ...
        for i, (real_imgs, real_labels) in enumerate(self.train_loader):
            ...
            if self.step % self.args.log_step == 0:
                print('step: {}, d_loss: {:.5f}, g_loss: {:.5f}'.format(self.step, d_loss.cpu().item(), g_loss.cpu().item()))
        ...
        return {'d_loss_real': d_loss_real.cpu().item(), 'd_loss_fake': d_loss_fake.cpu().item(),
                'd_loss': d_loss.cpu().item(), 'g_loss': g_loss.cpu().item()}
