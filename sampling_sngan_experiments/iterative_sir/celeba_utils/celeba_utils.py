...
def save_images_for_fid_fix_latent(
    ...
    for i, data_real in tqdm(enumerate(real_dataloader, 0)):
        ...
        if start_ind + batch_size < max_num_imgs:
            ...
            if normalize_imgs:
                fake_norm_np = (
                    ((1.0 + clamp_fake_images) / 2).detach().cpu().numpy()
                )
...
def calculate_celeba_statistics(
    z_agg_step,
    G,
    device,
    batch_size,
    ...
    for i in range(num_steps):
        ...
        latent_dataset = LatentFixDatasetCeleba(
            latent_arr,
            G,
            device,
            nsamples,
        ...
        if calculate_is:
            print(
                f"start to calculate inception score over {num_samples} images...",
            )
            start = time.time()
        ...
        print(
            f"start to calculate FID score for train Celeba over {num_samples} images...",
        )
        start = time.time()
        ...
        print(
            f"FID score for train Celeba with {method_name}: mean {mean_fid_train}, score {std_fid_train}",
        )
