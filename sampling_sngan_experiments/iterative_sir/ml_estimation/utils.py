...
def log_normal_full_cov(x, mean, L):
    ...
    def batch_inverse(M, damp=False, eps=1e-6):
        damp_matrix = Variable(
            torch.eye(M[0].size(0)).type(M.data.type()),
        ).mul_(eps)
        inverse = []
...
def discretized_logistic(mu, logs, x):
    ...
    logp = torch.log(
        sigmoid((x + 1.0 / 256.0 - mu) / s) - sigmoid((x - mu) / s) + 1e-7,
    )
...
def log_mean_exp(x):
    ...
    max_, _ = torch.max(x, 0, keepdim=True)
    return torch.log(torch.mean(torch.exp(x - max_), 0)) + torch.squeeze(max_)
