...
dset_dict = {
    ...
    "I32_hdf5": dset.ILSVRC_HDF5,
    "I64_hdf5": dset.ILSVRC_HDF5,
    "I128_hdf5": dset.ILSVRC_HDF5,
    "I256_hdf5": dset.ILSVRC_HDF5,
    "C10": dset.CIFAR10,
    "C100": dset.CIFAR100,
}
imsize_dict = {
...
root_dict = {
    "I32": "ImageNet",
    "I32_hdf5": "ILSVRC32.hdf5",
    "I64": "ImageNet",
    "I64_hdf5": "ILSVRC64.hdf5",
    "I128": "ImageNet",
    "I128_hdf5": "ILSVRC128.hdf5",
    "I256": "ImageNet",
    "I256_hdf5": "ILSVRC256.hdf5",
    "C10": "cifar",
    "C100": "cifar",
}
nclass_dict = {
...
class MultiEpochSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        num_epochs = int(
            np.ceil(
                (n * self.num_epochs - (self.start_itr * self.batch_size))
                / float(n),
            ),
        )
        ...
        # indices
        out = [torch.randperm(n) for epoch in range(self.num_epochs)][
            -num_epochs:
        ]
    ...
    def __len__(self):
        return (
            len(self.data_source) * self.num_epochs
            - self.start_itr * self.batch_size
        )
    ...
    **kwargs,
    ...
    if use_multiepoch_sampler:
        ...
        loader_kwargs = {
            ...
            "drop_last": drop_last,
        }  # Default, drop last incomplete batch
        train_loader = DataLoader(
            train_set,
            batch_size=batch_size,
            shuffle=shuffle,
...
def update_config_roots(config):
    if config["base_root"]:
        ...
        for key in ["data", "weights", "logs", "samples"]:
            config["%s_root" % key] = "{}/{}".format(config["base_root"], key)
    return config
...
def save_weights(
    ...
    torch.save(
        G.state_dict(),
        "{}/{}.pth".format(root, join_strings("_", ["G", name_suffix])),
    )
    torch.save(
        G.optim.state_dict(),
        "{}/{}.pth".format(root, join_strings("_", ["G_optim", name_suffix])),
    )
    torch.save(
        D.state_dict(),
        "{}/{}.pth".format(root, join_strings("_", ["D", name_suffix])),
    )
    torch.save(
        D.optim.state_dict(),
        "{}/{}.pth".format(root, join_strings("_", ["D_optim", name_suffix])),
    )
    torch.save(
        state_dict,
        "{}/{}.pth".format(
            root,
            join_strings("_", ["state_dict", name_suffix]),
        ),
    )
    if G_ema is not None:
        torch.save(
            ...
            "{}/{}.pth".format(
                root,
                join_strings("_", ["G_ema", name_suffix]),
            ),
        )
        ...
        if os.path.exists(f"{self.root}/{item}.log"):
            if self.reinitialize:
                ...
                if "sv" in item:
                    ...
                    print(
                        "{} exists, deleting...".format(
                            f"{self.root}_{item}.log",
                        ),
                    )
...
def write_metadata(logs_root, experiment_name, config, state_dict):
    with open(
        (f"{logs_root}/{experiment_name}/metalog.txt"),
        "w",
    ) as writefile:
            ...
            if n > 0:
                ...
                if displaytype == "s1k":  # minutes/seconds for 1000 iters
                    ...
                    outlist = list(divmod(t_done, 60)) + list(
                        divmod(t_1k - t_done, 60),
                    )
                    print(
                        "(TE/ET1k: %d:%02d / %d:%02d)" % tuple(outlist),
                        end=" ",
                    )
                    ...
                    print(
                        "(TE/ETA: %d:%02d / %d:%02d)" % tuple(outlist),
                        end=" ",
                    )
...
def sample_sheet(
    ...
    if not os.path.isdir(
        "%s/%s/%d" % (samples_root, experiment_name, folder_number),
    ):
    ...
    name = "_".join(
        [
            ...
            for item in [
                ...
                "GBB%3.3f" % config["G_B2"]
                if config["G_B2"] != 0.999
                else None,
                "DB%3.3f" % config["D_B1"] if config["D_B1"] != 0.0 else None,
                "DBB%3.3f" % config["D_B2"]
                if config["D_B2"] != 0.999
                else None,
                "Gnl%s" % config["G_nl"],
                "Dnl%s" % config["D_nl"],
                ...
                "G%s" % config["G_param"]
                if config["G_param"] != "SN"
                else None,
                "D%s" % config["D_param"]
                if config["D_param"] != "SN"
                else None,
                "Gattn%s" % config["G_attn"]
                if config["G_attn"] != "0"
                else None,
                "Dattn%s" % config["D_attn"]
                if config["D_attn"] != "0"
                else None,
                "Gortho%2.1e" % config["G_ortho"]
                if config["G_ortho"] > 0.0
                else None,
                "Dortho%2.1e" % config["D_ortho"]
