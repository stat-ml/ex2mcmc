...
import legacy
import torch
from metrics_stylegan2_ada import metric_main, metric_utils
...
def calc_metrics(ctx, network_pkl, metrics, data, mirror, gpus, verbose):
    ...
    """
    dnnlib.util.Logger(should_flush=True)
    print(f"list of valid metrics = {metric_main.list_valid_metrics()}")
    ...
    if data is not None:
        ...
        raise ValueError(
            "Could not look up dataset options; please specify --data",
        )
