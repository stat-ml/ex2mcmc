...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    if not training_set.has_labels:
        ...
        grid_indices = [
            all_indices[i % len(all_indices)] for i in range(gw * gh)
        ]
...
def training_loop(
    ...
    # Enable torch.backends.cuda.matmul.allow_tf32 and torch.backends.cudnn.allow_tf32?
    allow_tf32=False,
    ...
    torch.backends.cudnn.benchmark = (
        cudnn_benchmark  # Improves training speed.
    )
    ...
    grid_sample_gradfix.enabled = (
        True  # Avoids errors with the augmentation pipe.
    )
    ...
    augment_pipe = None
    ada_stats = None
    if (augment_kwargs is not None) and (
        augment_p > 0 or ada_target is not None
    ):
        augment_pipe = (
    ...
    if rank == 0:
        ...
        grid_z = torch.randn([labels.shape[0], G.z_dim], device=device).split(
            batch_gpu,
        )
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function("data_fetch"):
            ...
            all_gen_c = (
                torch.from_numpy(np.stack(all_gen_c)).pin_memory().to(device)
            )
            all_gen_c = [
        ...
        for phase, phase_gen_z, phase_gen_c in zip(
            phases,
            all_gen_z,
            all_gen_c,
        ):
