...
class AugmentPipe(torch.nn.Module):
    ...
    def forward(self, images, debug_percentile=None):
        ...
        if self.xint > 0:
            t = (
                torch.rand([batch_size, 2], device=device) * 2 - 1
            ) * self.xint_max
            t = torch.where(
        ...
        if G_inv is not I_3:
            ...
            margin = (
                cp[:, :2, :].permute(1, 0, 2).flatten(1)
            )  # [xy, batch * idx]
            margin = (
                torch.cat([-margin, margin]).max(dim=1).values
            )  # [x0, y0, x1, y1]
            margin = margin + misc.constant(
                [Hz_pad * 2 - cx, Hz_pad * 2 - cy] * 2,
                device=device,
            )
        ...
        if C is not I_4:
            ...
            if num_channels == 3:
                ...
                images = (
                    images * C[:, :, :3].sum(dim=2, keepdims=True)
                    + C[:, :, 3:]
                )
            else:
                raise ValueError(
        ...
        if self.noise > 0:
            sigma = (
                torch.randn([batch_size, 1, 1, 1], device=device).abs()
                * self.noise_std
            )
            sigma = torch.where(
        ...
        if self.cutout > 0:
            ...
            coord_y = torch.arange(height, device=device).reshape(
                [1, 1, -1, 1],
            )
            mask_x = ((coord_x + 0.5) / width - center[:, 0]).abs() >= size[
                :, 0
            ] / 2
            mask_y = ((coord_y + 0.5) / height - center[:, 1]).abs() >= size[
                :, 1
            ] / 2
