...
class Generator(nn.Module):
    def __init__(self, ngpu, nc=1, nz=100, ngf=64):
        ...
        self.main = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.ConvTranspose2d(
                ngf, nc, kernel_size=1, stride=1, padding=2, bias=False
            ),
            nn.Tanh(),
        )
...
class Discriminator(nn.Module):
    ...
    def forward(self, input):
        ...
        return output.view(-1)  # .squeeze(1)
...
if __name__ == "__main__":
    ...
    for epoch in range(opt.niter):
        ...
        # do checkpointing
        torch.save(
            netG.state_dict(), "%s/netG_epoch_%d.pth" % (opt.outf, epoch)
        )
        torch.save(
            netD.state_dict(), "%s/netD_epoch_%d.pth" % (opt.outf, epoch)
        )
