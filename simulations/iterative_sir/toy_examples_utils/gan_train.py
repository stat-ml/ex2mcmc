...
from iterative_sir.sampling_utils.visualization import epoch_visualization
...
def d_loss(fake, real, loss_type="Jensen_nonsaturing", criterion=None):
    if loss_type == "Hinge":
        ...
        d_fake_loss = criterion(fake, 1.0 - one_b)
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for data in train_dataloader:
                ...
                d_optimizer.step()
                discriminator_loss_arr.append(
                    discriminator_loss.data.cpu().item(),
                )
            ...
            discriminator_mean_loss_arr.append(
                np.mean(discriminator_loss_arr[-k_d:]),
            )
            ...
            discriminator_msg = (
                "Discriminator last mean loss: \t{:.6f}\n".format(
            ...
            if epoch % num_epoch_for_save == 0:
                ...
                if path_to_models is not None:
                    ...
                    path_to_discriminator = os.path.join(
                        path_to_models,
