...
def xtry_langevin_dynamics(
    ...
    for _ in range(n_steps):
        ...
        T_x = torch.transpose(
            T_x.reshape(list(X.shape[:-1][::-1]) + [X.shape[-1]]),
            0,
            1,
        )
        ...
        sum_log_gauss = log_gauss.sum(1) - torch.diagonal(
            log_gauss,
            dim1=1,
            dim2=2,
        )
...
class MHKernel:
    ...
    def log_prob(
        ...
        return (log_pi2 + log_transition_backward) - (
            log_pi1 + log_transition_forward
        )
...
class MALATransition:
    ...
    def __call__(
        ...
        z_new = (
            z
            - grad_step * beta * grad
            + sigma * self.stand_normal.sample(z.shape[:-1])
        )
...
def i_ais_z_dynamics(z, target, n_steps, grad_step, eps_scale, N, betas):
    ...
    for step in tqdm(range(n_steps)):
        ...
        grad = (
            torch.transpose(grad, 0, 1).reshape((batch_size * N_, z_dim)).data
        )
        # detach().clone()
        ...
        max_logs = torch.max(log_weights, dim=1)[
            0
        ]  # .unsqueeze(-1).repeat((1, N))
        ...
        energy_backward_last = energy_backward[
...
def citerais_mala_dynamics(
    ...
    for _ in trange(n_steps):
        ...
        for t in range(1, T + 1):
            ...
            not_equal_mask = torch.ne(z[:, t, :], z[:, t - 1, :]).max(dim=-1)[
                0
            ]
